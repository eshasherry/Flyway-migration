...
VALUES
  ('tee','export_users','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
...
VALUES
  ('tee','export_users','','','en','Export Users','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
...
VALUES
  ('tee_api','tee','export_users','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
...
VALUES
  ('users_export_tee_ids_upsert',0,'Initial TEE Query for User Export (Retrieve User List)',   'ttc_verify',NOW(),'/artifacts/tee_default_user_export_ids_upsert.xml',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
  ('users_export_tee_pfl_fetch', 0,'Initial TEE Query for User Export (Retrieve User Details)','ttc_verify',NOW(),'/artifacts/tee_default_user_export_pfl_fetch.xml', 0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
  ('mirroring_users_tee_agent',  0,'Initial TEE Transform for User Mirroring by Agent',        'ttc_verify',NOW(),'/artifacts/tee_default_user_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
...
VALUES
  ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_TEE','UPDATE','mirror_users','mirroring_users_tee_agent');
