DELIMITER //
CREATE PROCEDURE provision_infor_temp_160()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('infor','export_jobs','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('infor','export_jobs','','','en','Export Jobs','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('infor_api','infor','export_jobs','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('jobs_add_infor_agent',0,'Initial Infor Transform for Job Add by Agent',   'ttc_verify',NOW(),'/artifacts/infor_default_job_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('jobs_del_infor_agent',0,'Initial Infor Transform for Job Delete by Agent','ttc_verify',NOW(),'/artifacts/infor_default_job_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_INFOR','ADD','transform_jobs','jobs_add_infor_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_INFOR','DEL','transform_jobs','jobs_del_infor_agent');
    END IF;
END//
DELIMITER ;

CALL provision_infor_temp_160();
DROP PROCEDURE provision_infor_temp_160;
