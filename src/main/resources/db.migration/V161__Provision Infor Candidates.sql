DELIMITER //
CREATE PROCEDURE provision_infor_temp_161()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('infor','export_candidates','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('infor','export_candidates','','','en','Export Candidates','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('infor_api','infor','export_candidates','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('candidates_add_infor_agent',0,'Initial Infor Transform for Candidate Add by Agent',   'ttc_verify',NOW(),'/artifacts/infor_default_candidate_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('candidates_del_infor_agent',0,'Initial Infor Transform for Candidate Delete by Agent','ttc_verify',NOW(),'/artifacts/infor_default_candidate_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_INFOR','ADD','transform_candidates','candidates_add_infor_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_INFOR','DEL','transform_candidates','candidates_del_infor_agent');
    END IF;
END//
DELIMITER ;

CALL provision_infor_temp_161();
DROP PROCEDURE provision_infor_temp_161;
