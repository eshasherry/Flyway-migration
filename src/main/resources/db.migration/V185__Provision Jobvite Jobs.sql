DELIMITER //
CREATE PROCEDURE provision_jobvite_temp_185()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('jobvite','export_jobs','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('jobvite','export_jobs','','','en','Export Jobs','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('jobvite_api','jobvite','export_jobs','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('jobs_add_jobvite_agent',0,'Initial Jobvite Transform for Job Add by Agent',   'ttc_verify',NOW(),'/artifacts/jobvite_default_job_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('jobs_del_jobvite_agent',0,'Initial Jobvite Transform for Job Delete by Agent','ttc_verify',NOW(),'/artifacts/jobvite_default_job_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_JOBVITE','ADD','transform_jobs','jobs_add_jobvite_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_JOBVITE','DEL','transform_jobs','jobs_del_jobvite_agent');
    END IF;
END//
DELIMITER ;

CALL provision_jobvite_temp_185();
DROP PROCEDURE provision_jobvite_temp_185;
