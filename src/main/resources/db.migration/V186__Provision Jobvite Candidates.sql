DELIMITER //
CREATE PROCEDURE provision_jobvite_temp_186()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('jobvite','export_candidates','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('jobvite','export_candidates','','','en','Export Candidates','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('jobvite_api','jobvite','export_candidates','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('candidates_add_jobvite_agent',0,'Initial Jobvite Transform for Candidate Add by Agent',   'ttc_verify',NOW(),'/artifacts/jobvite_default_candidate_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('candidates_del_jobvite_agent',0,'Initial Jobvite Transform for Candidate Delete by Agent','ttc_verify',NOW(),'/artifacts/jobvite_default_candidate_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_JOBVITE','ADD','transform_candidates','candidates_add_jobvite_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_JOBVITE','DEL','transform_candidates','candidates_del_jobvite_agent');
    END IF;
END//
DELIMITER ;

CALL provision_jobvite_temp_186();
DROP PROCEDURE provision_jobvite_temp_186;
