...
DELIMITER //
CREATE PROCEDURE provision_successfactors_temp_191()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('sf','Can9YfLjUrFX0b2Cj7uAsD/GvyeJCgqnj1IabodVpe0dl9EEMamipDrDHnDvnX/q','SAP SuccessFactors', 'integration', 1);
        ...
        VALUES
            ('sf_api','ttc_verify','{"":""}',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('sf_api','','','en','API only',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        
        INSERT INTO `service_definitions`
            (`service_tp_id`,`service_id`,`service_category_id`,`repeatable`,`vendor_service_id`,`created_date`,`created_tp_id`
            ,`created_tp_user_id`,`updated_date`,`updated_tp_id`,`updated_tp_user_id`)
        VALUES
            ('sf','_core',       '_core',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify'),
            ('sf','_core_vendor','_core',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        INSERT INTO `service_texts`
            ...
            ,`updated_date`,`updated_tp_id`,`updated_tp_user_id`)
        VALUES
            ('sf','_core',       '','','en','Built-in',         '','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify'),
            ('sf','_core_vendor','','','en','Built-in (Vendor)','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('sf_api','sf','_core',       '','',2,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1),
            ('sf_api','sf','_core_vendor','','',2,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
    END IF;
END//
DELIMITER ;

CALL provision_successfactors_temp_191();
DROP PROCEDURE provision_successfactors_temp_191;
