DELIMITER //
CREATE PROCEDURE provision_successfactors_temp_193()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('sf','export_candidates','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('sf','export_candidates','','','en','Export Candidates','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('sf_api','sf','export_candidates','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('candidates_add_sf_agent',0,'Initial SuccessFactors Transform for Candidate Add by Agent',   'ttc_verify',NOW(),'/artifacts/sf_default_candidate_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('candidates_del_sf_agent',0,'Initial SuccessFactors Transform for Candidate Delete by Agent','ttc_verify',NOW(),'/artifacts/sf_default_candidate_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_SF','ADD','transform_candidates','candidates_add_sf_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_SF','DEL','transform_candidates','candidates_del_sf_agent');
    END IF;
END//
DELIMITER ;

CALL provision_successfactors_temp_193();
DROP PROCEDURE provision_successfactors_temp_193;
