DELIMITER //
CREATE PROCEDURE provision_orc_temp_204()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('orc','export_jobs','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('orc','export_jobs','','','en','Export Jobs','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('orc_api','orc','export_jobs','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('jobs_add_orc_agent',0,'Initial ORC Transform for Job Add by Agent',   'ttc_verify',NOW(),'/artifacts/orc_default_job_export_add_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('jobs_del_orc_agent',0,'Initial ORC Transform for Job Delete by Agent','ttc_verify',NOW(),'/artifacts/orc_default_job_export_del_agent.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_ORC','ADD','transform_jobs','jobs_add_orc_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_ORC','DEL','transform_jobs','jobs_del_orc_agent');
    END IF;
END//
DELIMITER ;

CALL provision_orc_temp_204();
DROP PROCEDURE provision_orc_temp_204;
