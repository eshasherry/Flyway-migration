DELIMITER //
CREATE PROCEDURE provision_csv_hire_candidates_213()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('csvhire','export_candidates','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('csvhire','export_candidates','','','en','Export Hire Report','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('csv_hire_api','csvhire','export_candidates','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        ...
        VALUES
            ('candidates_add_csv_hire_agent',0,'Initial CSV Hire Transform for Candidate Add by Agent',   'ttc_verify',NOW(),'/artifacts/generic_default_candidate_export_add_csv.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway'),
            ('candidates_del_csv_hire_agent',0,'Initial CSV Hire Transform for Candidate Delete by Agent','ttc_verify',NOW(),'/artifacts/generic_default_candidate_export_del_csv.xslt',0,'com.talemetry.verify.serviceprovider.talemetryapigateway');
        ...
        VALUES
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_CSV_HIRE','ADD','transform_candidates','candidates_add_csv_hire_agent'),
            ('ttc_api_gateway','test_client_ttc','','ttc_agent','SYS_CSV_HIRE','DEL','transform_candidates','candidates_del_csv_hire_agent');
    END IF;
END//
DELIMITER ;

CALL provision_csv_hire_candidates_213();
DROP PROCEDURE provision_csv_hire_candidates_213;