DELIMITER //
CREATE PROCEDURE provision_peoplesofthcm_temp_217()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES ('peoplesofthcm', 'new_hire_export', 'integration', '0', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
        ...
        VALUES ('peoplesofthcm', 'new_hire_export', '', '', 'en', 'New Hire Export (JV to PSFT)', '', '', '', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
        ...
        VALUES
            ('peoplesofthcm_api','peoplesofthcm','new_hire_export','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
            
    END IF;
END//
DELIMITER ;

CALL provision_peoplesofthcm_temp_217();
DROP PROCEDURE provision_peoplesofthcm_temp_217;