...
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES ('peoplesofthcm', 'field_import', 'integration', '0', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
        ...
        VALUES ('peoplesofthcm', 'field_import', '', '', 'en', 'Field Import (PSFT to JV)', '', '', '', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
	    ...
	    VALUES
            ('peoplesofthcm_api','peoplesofthcm','field_import','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
    END IF;
END//
DELIMITER ;
