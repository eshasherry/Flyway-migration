DELIMITER //
CREATE PROCEDURE provision_jobviteapigateway_temp_228()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES ('jobviteapigateway', 'employee_import', 'integration', '0', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
        ...
        VALUES ('jobviteapigateway', 'employee_import', '', '', 'en', 'Employee Import', '', '', '', NOW(), 'ttc_verify', 'ttc_verify', NOW(), 'ttc_verify', 'ttc_verify');
		...
		VALUES
            ('jobviteapigateway_api','jobviteapigateway','employee_import','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);

    END IF;
END//
DELIMITER ;

CALL provision_jobviteapigateway_temp_228();
DROP PROCEDURE provision_jobviteapigateway_temp_228;