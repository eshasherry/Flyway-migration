DELIMITER //
CREATE PROCEDURE provision_csv_candidates_269()
BEGIN
    ...
    IF var_count = 1 THEN
        ...
        VALUES
            ('csv','export_employees','integration',0,'',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('csv','export_employees','','','en','Export Employees','','','',NOW(),'ttc_verify','ttc_verify',NOW(),'ttc_verify','ttc_verify');
        ...
        VALUES
            ('csv_api','csv','export_employees','','',0,1,1,0,0,now(),'ttc_verify','ttc_verify',now(),'ttc_verify','ttc_verify',-1,-1,0,1);
        
         END IF;
END//
DELIMITER ;

CALL provision_csv_candidates_269();
DROP PROCEDURE provision_csv_candidates_269;